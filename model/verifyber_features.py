...
class DECSeq(torch.nn.Module):
    ...
    def forward(self, data):
        df, bvec, eidx = data.x, data.bvec, data.edge_index

        torch.cuda.synchronize()
        t = time.time()
        x1 = self.conv1(df, eidx)

        torch.cuda.synchronize()
        print(f"conv1: {time.time() - t:.2f}s")
        t = time.time()

        x2 = self.conv2(x1, bvec)

        torch.cuda.synchronize()
        print(f"conv2 DynamicEdgeConv: {time.time() - t:.2f}s")
        t = time.time()

        out = self.lin1(torch.cat([x1, x2], dim=1))

        torch.cuda.synchronize()
        print(f"lin1: {time.time() - t:.2f}s")
        t = time.time()

        out = global_max_pool(out, data.bvec)

        torch.cuda.synchronize()
        print(f"pool: {time.time() - t:.2f}s")
        t = time.time()

        out = self.mlp(out)

        torch.cuda.synchronize()
        print(f"mlp: {time.time() - t:.2f}s")
