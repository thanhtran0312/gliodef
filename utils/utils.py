...
def irbio_path(bundle_idx,bundle):
    old_root = '/nilab-nexus'
    new_root = '/home/thuythienthanh.tran/mnt/mount_point'
    for i in range(len(bundle_idx[bundle])):
        bundle_idx[bundle][i]['path'] = bundle_idx[bundle][i]['path'].replace(old_root,new_root)
    return bundle_idx
    
def digis_path(bundle_idx,bundle):
    """for every subject of the bundle"""
    old_root = '/nilab-nexus/datasets/GLIODEF'
    new_root = '/app/data'   # wherever your data actually lives now
    for j in range(len(bundle_idx[bundle])):
        bundle_idx[bundle][j]['path'] = bundle_idx[bundle][j]['path'].replace(old_root, new_root)
    return bundle_idx

def change_to_trk(path_bundle):
